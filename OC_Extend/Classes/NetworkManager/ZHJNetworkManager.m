...
//        self.sessionManager.securityPolicy = securityPolicy;
        ...
        // 设置超时时间
        self.sessionManager.requestSerializer.timeoutInterval = 25.0;
        // 设置响应内容的类型
        [self.sessionManager.requestSerializer setValue:@"multipart/form-data" forHTTPHeaderField:@"Content-Type"];
        [self.sessionManager.requestSerializer setValue:@"ZwfWiEWrgEFA9A785H12weF7106AJ" forHTTPHeaderField:@"boundary"];
...
- (nullable NSURLSessionDataTask *)sendRequestMethod:(HTTPMethod)requestMethod
    ...
    switch (requestMethod) {
        case HTTPMethodGET:
        {
            task = [weakSelf.sessionManager GET:requestPath parameters:parameters progress:progress success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
                if (success) {
