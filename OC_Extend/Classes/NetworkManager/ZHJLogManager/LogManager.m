...
#pragma mark - 获取参数
    ...
    @try {
        ...
        [parmaStr appendString:@"【开始记录日志异常】"];
    } @finally {
        // 将参数列表指针置空
        va_end(paramList);
    }
...
//
//    // 请求的URL，后台功能需要自己做
//    NSString* url = [NSString stringWithFormat:@"https://auth.zjdex.com/common/phone/logs"];
//
//    // 发起请求，从服务器上获取当前应用是否需要上传日志
//    [XMNetWorkHelper getWithUrlString:url parameters:nil success:^(NSDictionary *data) {
//        // 获取实体字典
...
- (void)uploadLog:(NSDictionary*)resultDic{
    ...
    if (created) {
        ...
        [self uploadLogToServer:^(BOOL boolValue) {
            ...
            NSLog(@"%@",[NSString stringWithFormat:@"日志上传失败！！Error:%@",errorInfo]);
        }];
    }
}
...
//     访问URL
//    NSString* url = [NSString stringWithFormat:@"https://auth.zjdex.com/fileupload/fileupload/logs"];
    
    // 压缩包文件路径
//    NSString * zipFile = [self.basePath stringByAppendingString:ZipFileName] ;
    
//     发起请求，这里是上传日志到服务器,后台功能需要自己做
//    NSData *fileData0 = [NSData dataWithContentsOfFile:zipFile];
    
//    NSData *fileData1 = [[NSFileManager defaultManager] contentsAtPath:zipFile];
...
+(NSString*)getClassForCoderName:(id)currentClass {
    return [NSString stringWithFormat:@"%@",currentClass];
}
